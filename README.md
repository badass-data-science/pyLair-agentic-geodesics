...
| `-t` | `--truncation` | Cutoff ratio (0-1) from the minimum Z (vertical) extent; keeps the portion above this fraction, discards the rest. Passing this enables Z-axis truncation. Z-only truncation (no `-x`/`-y`) now correctly clips face data too, so it's compatible with `-F`/`-s`/`-O`/`-T`/`-a`/`-w`. | off (full sphere) |
| `-x` | `--truncation-x` | Same rule as `-t`, but along X. Off by default; combine with `-t`/`-y` to clip more than one axis (applied in X, then Y, then Z order, so a later axis's cutoff is computed against that axis's range *after* earlier axes are already trimmed). Unlike Z-only truncation, X/Y truncation does not yet preserve face data, so it's still incompatible with `-F`/`-s`/`-O`/`-T`/`-a`/`-w`. | off |
...
| `-F` | `--face` | Emit face data (not wireframe) in the WRL output; skips DXF entirely. Incompatible with `-x`/`-y` truncation; works fine with Z-only `-t`. | off |
...
| `-s` | `--stl` | Also save an STL file (`<output>.stl`) of the dome's surface triangles, e.g. for 3D-printing a scale model. Requires face data; incompatible with `-x`/`-y` truncation; works fine with Z-only `-t`. | off |
| `-O` | `--obj` | Also save an OBJ file (`<output>.obj`) of the dome's surface triangles. Requires face data; incompatible with `-x`/`-y` truncation; works fine with Z-only `-t`. | off |
...
| `-T` | `--face-templates` | Also save one 2D DXF cutting template per unique panel (face) shape (`<output>_facetype1.dxf`, `<output>_facetype2.dxf`, ...), for laser-cutting/CNC panel material. Requires face data; incompatible with `-x`/`-y` truncation; works fine with Z-only `-t`. | off |
| `-a` | `--area-cost` | Price per unit area of panel material. If given, adds an estimated total panel material cost to the report alongside the total panel area (both reported whenever face data is available). Requires face data; incompatible with `-x`/`-y` truncation; works fine with Z-only `-t`. Must be > 0. | off (area only) |
| `-w` | `--panel-density` | Areal density (mass per unit area, e.g. kg/m²) of panel material. If given, adds an estimated total panel weight to the report. Requires face data; incompatible with `-x`/`-y` truncation; works fine with Z-only `-t`. Must be > 0. | off |
...
- **`-F/--face`, `-s/--stl`, `-O/--obj`, `-T/--face-templates`, `-a/--area-cost`, and `-w/--panel-density` all require face data.** Z-only truncation (`-t` alone, no `-x`/`-y`) now clips the face list against the same cutoff plane used for vertices/chords — reusing the exact same edge-intersection points, so a clipped panel's new corner and the strut running along that same cut land on the identical point rather than two independently-rounded near duplicates — so it's compatible with all of the above. `-x`/`-y` truncation, though, still discards face data entirely (`F_sphere` becomes `None`): the clipping logic itself is already generic across any single axis, but sequential multi-axis face clipping — where a triangle already split by one axis's cut gets split again by another — hasn't been implemented/tested yet, so the CLI still rejects `-x`/`-y` combined with any face-based output rather than risk silently wrong geometry. This is a deliberately staged rollout, not a permanent limitation.
...
| `pydome/api.py` | Programmatic entry point shared by the CLI and the MCP server: `build_dome(...)` (validation, symmetry-triangle construction, projection, elongation, sequential X/Y/Z truncation) and the shared `ValueError`-raising validators. |
...
| `pydome/truncation.py` | Cuts a geodesic sphere at a plane perpendicular to a given axis (`-t/-x/-y`, applied independently per axis) to produce a dome, including clipping the face list (for Z-only truncation so far) into correctly-shaped smaller triangles along the cutoff. |
| `pydome/elongation.py` | Scales all three axes independently (`-e/--elongation`) to turn the sphere into a general axis-aligned ellipsoid, for ceiling-height/footprint tradeoffs on any axis. |
