...
An exposed lattice of hubs and struts is appropriately villainous, but not exactly draft-proof, so the bill of materials extends the same treatment to the dome's actual *skin* whenever the structure is left untruncated (truncation only recomputes struts, not the panels sitting between them, so all of the following politely disappears from the report the moment the user truncates along any axis). Every triangular panel gets grouped by its three edge lengths, the same way strut lengths are grouped, and pyDome reports how many of each shape are needed — flagging, along the way, something our heroine hadn't originally considered: two panels with identical edge lengths can still be mirror images of each other rather than true duplicates, which matters if the panel material itself has a direction to it (wood grain, a printed pattern, a one-sided finish). The report also totals the panel area, with optional per-unit-area cost and areal-density weight estimates matching the strut side's own cost arithmetic, and lists a bevel angle for every strut in the structure — the angle each bordering panel's edge needs to be cut at so two flat panels actually meet flush along that strut instead of leaving a gap or an overlap. Since a wrong dihedral-angle formula would mean panels that quietly don't fit, this math was checked against the textbook dihedral angles of a plain icosahedron and octahedron (matching to six decimal places) before our heroine trusted it, and got a second opinion from two independent libraries described under Works Consulted below.
