...
Chapter 15 already generates exactly the file pyFit needs: `-T/--face-templates` writes one DXF per distinct panel shape, and the same report that triggers those files carries a **"Panel Cutting Templates"** section pairing each one with its own real panel count:
